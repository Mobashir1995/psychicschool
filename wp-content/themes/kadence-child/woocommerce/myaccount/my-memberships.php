						...
						<?php if ( 'membership-plan' === $column_id ) : ?>
							...
							<td class="membership-plan" data-title="<?php echo esc_attr( $column_name ); ?>">
								...
								<?php if ( ( ! empty ( $members_area ) && is_array( $members_area ) ) && ( wc_memberships_is_user_active_member( get_current_user_id(), $customer_membership->get_plan() ) || wc_memberships_is_user_delayed_member( get_current_user_id(), $customer_membership->get_plan() ) ) ) : ?>
									...
									<?php
										$restricted_content = $customer_membership->get_plan()->get_restricted_content(1);
										$membership_permalink =  wc_memberships_get_members_area_url( $customer_membership->get_plan_id(), $default_section );
										...
										if( is_object( $restricted_content ) && ! empty( $restricted_content ) ) {
											if( isset( $restricted_content->posts ) && is_array( $restricted_content->posts ) && !empty( $restricted_content->posts ) ) {
												...
												$membership_permalink = get_permalink( $restricted_page->ID );
											}
										}
										
									?>

									<a href="<?php echo esc_url( $membership_permalink ); ?>"><?php echo esc_html( $customer_membership->get_plan()->get_name() ); ?></a>

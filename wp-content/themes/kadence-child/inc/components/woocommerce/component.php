...
/**
 * Single product: remove tabs, reorder via custom hooks. Summary keeps only price and add-to-cart.
 */
function kadence_child_single_product_layout_hooks() {
	if ( ! is_product() ) {
		return;
	}
...
add_action( 'wp', 'kadence_child_single_product_layout_hooks', 20 );

/**
 * Single product header: title + rating (hooked into kadence_child_single_product_header).
 */
function kadence_child_single_product_header_content() {
	echo '<div class="single-product-header">';
	woocommerce_template_single_title();
	woocommerce_template_single_rating();
	echo '</div>';
}
add_action( 'kadence_child_single_product_header', 'kadence_child_single_product_header_content', 10 );

/**
 * Single product left column: description and additional info tab content (no tabs UI).
 */
function kadence_child_single_product_description_content() {
	$tabs = apply_filters( 'woocommerce_product_tabs', array() );
	if ( ! empty( $tabs['description'] ) && is_callable( $tabs['description']['callback'] ) ) {
		echo '<div class="woocommerce-product-details__short-description">';
		call_user_func( $tabs['description']['callback'], 'description', $tabs['description'] );
		echo '</div>';
	}
	if ( ! empty( $tabs['additional_information'] ) && is_callable( $tabs['additional_information']['callback'] ) ) {
		echo '<div class="woocommerce-product-details__additional-information">';
		call_user_func( $tabs['additional_information']['callback'], 'additional_information', $tabs['additional_information'] );
		echo '</div>';
	}
}
add_action( 'kadence_child_single_product_description', 'kadence_child_single_product_description_content', 10 );

/**
 * Single product reviews at bottom (same content as reviews tab).
 */
function kadence_child_single_product_reviews_content() {
	$tabs = apply_filters( 'woocommerce_product_tabs', array() );
	if ( ! empty( $tabs['reviews'] ) && is_callable( $tabs['reviews']['callback'] ) ) {
		call_user_func( $tabs['reviews']['callback'], 'reviews', $tabs['reviews'] );
	}
}
add_action( 'kadence_child_single_product_reviews', 'kadence_child_single_product_reviews_content', 10 );

/**
 * Single product: remove quantity input (force 1, hide with CSS).
 */
function kadence_child_single_product_quantity_args( $args, $product ) {
	if ( is_product() ) {
		$args['min_value']   = 1;
		$args['max_value']   = 1;
		$args['input_value'] = 1;
...
add_filter( 'woocommerce_quantity_input_args', 'kadence_child_single_product_quantity_args', 10, 2 );

...
add_action( 'init', array( __NAMESPACE__ . '\STM_PostType', 'init' ), 1 );
...
class STM_PostType {
	...
	public static function registerPostType( $postType, $title, $args ) {
		...
		$labels      = array(
			'name'               => __( $pluralTitle, 'stm-post-type' ),
			'singular_name'      => __( $title, 'stm-post-type' ),
			'add_new'            => __( 'Add New', 'stm-post-type' ),
			'add_new_item'       => __( 'Add New ' . $title, 'stm-post-type' ),
			'edit_item'          => __( 'Edit ' . $title, 'stm-post-type' ),
			'new_item'           => __( 'New ' . $title, 'stm-post-type' ),
			'all_items'          => __( 'All ' . $pluralTitle, 'stm-post-type' ),
			'view_item'          => __( 'View ' . $title, 'stm-post-type' ),
			'search_items'       => __( 'Search ' . $pluralTitle, 'stm-post-type' ),
			'not_found'          => __( 'No ' . $pluralTitle . ' found', 'stm-post-type' ),
			'not_found_in_trash' => __( 'No ' . $pluralTitle . '  found in Trash', 'stm-post-type' ),
			'parent_item_colon'  => '',
			'menu_name'          => __( $pluralTitle, 'stm-post-type' )
		);
		...
		$is_elementor = false;
		if (is_admin() || current_user_can('administrator')) $is_elementor = true;
	...
	public static function addMetaBox( $id, $title, $post_types, $callback = '', $context = '', $priority = '', $callback_args = '' ) {
		...
		foreach ( $post_types as $post_type ) {
			$title                                     = empty( $title ) ? __( 'Options', 'stm-post-type' ) : $title;
		...
		foreach ( $fields as $key => $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'images':
					...
					echo '</div>'.
						 '<div class="stm-add-media button button-primary">'.__( "Add", 'stm-post-type' ).'</div>'.
						 '<script>
						    jQuery(function($) {
				...
				case 'multi-select':
					...
					foreach ( $field['options'] as $key => $value ) {
						if( is_array($meta) and in_array($key, $meta) or $key == $meta ){
				...
				case 'image':
					...
					echo '
						<div class="stm_metabox_image">
							...
							<input class="stm_upload_image upload_button_'. $key .' button-primary" type="button" value="' . __( 'Choose Image', 'stm-post-type' ). '" />
							<a href="#" class="stm_remove_image button">' . __( 'Remove Image', 'stm-post-type' ). '</a>
						</div>
						<script type="text/javascript">
							jQuery(function($) {
								$(".upload_button_'. $key .'").click(function(){
									...
									var custom_uploader = wp.media({
										title   : "' . __( 'Select image', 'stm-post-type' ) . '",
										button  : {
											text: "' . __( 'Attach', 'stm-post-type' ) . '"
										},
										multiple: true
									}).on("select", function () {
